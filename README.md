...
Студент: Поздняков Егор Игоревич
...
На скриншоте показана регистрация пользователя с email формата `student_surname@email.com`, как требуется в задании.

![Регистрация пользователя](screenshots/01_register.png)

### 2. Логин и получение JWT
На скриншоте показан успешный вход пользователя и получение JWT access token.

![Логин и получение JWT](screenshots/02_login_jwt.png)

### 3. Авторизация через Swagger
На скриншоте показана авторизация через кнопку **Authorize** в Swagger UI.

![Авторизация через Swagger](screenshots/03_authorize.png)

### 4. Вызов POST /chat
На скриншоте показан успешный вызов защищённого endpoint `POST /chat` и ответ модели.

![Вызов POST /chat](screenshots/04_chat.png)

### 5. Получение истории через GET /chat/history
На скриншоте показано, что история сообщений сохраняется и возвращается пользователю.

![Получение истории](screenshots/05_history.png)

### 6. Удаление истории через DELETE /chat/history
На скриншоте показана успешная очистка истории сообщений.

![Удаление истории](screenshots/06_delete_history.png)
